...
io.on("connection", (socket) => {
  ...
  socket.on("createRoom", async ({ roomName }) => {
    try {
      ...
      console.log(`createroom socket rooms: ${socket.rooms}`);
      let room = new Room();
      console.log(`socket id is ${socket.user}`);
      let player = {
        socketId: socket.id,
        nickName: roomName,
        playerType: "X",
      ...
      console.log(`error from create room : ${e.toString()}`);
    }
  });
  ...
  socket.on("leaveRoom", async ({ roomId }) => {
    ...
    try {
      ...
      io.to(roomId).emit("playerLeft", "Player left the game");
      socket.leaveAll();
    } catch (e) {
      console.log(e.toString());
    }
